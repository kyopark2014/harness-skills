...
CloudFront, IAM roles, Memory, S3 Vectors Knowledge Base, KB + S3-sharing
...
# Configuration
project_name = "harness-skills"  # at least 3 characters
region = "us-west-2"
...
# ---------------------------------------------------------------------------
# Knowledge Base MCP (AgentCore Runtime, IAM auth)
# ---------------------------------------------------------------------------
...
BASE_SYSTEM_PROMPT = (
    ...
    "사용자에게 최종 답변하기 **전에** 반드시 artifact-share MCP의 share_artifact를 호출하세요.\n"
...
def main():
    ...
    try:
        ...
        logger.info(
            ...
            f"{knowledge_base_mcp_info.get('agent_runtime_arn')}"
        )
        logger.info(
        ...
        logger.info(
            f"  AgentCore Gateway: "
            f"{knowledge_base_mcp_info.get('agentcore_gateway_arn')}"
        )
        logger.info(
            f"  Knowledge Base MCP URL: "
            f"{knowledge_base_mcp_info.get('knowledge_base_mcp_url')}"
        )
        logger.info(
        ...
        logger.info(
            ...
            "and the password set during install"
        )
        logger.info("=" * 60)
    except Exception as e:
        elapsed_time = time.time() - start_time
