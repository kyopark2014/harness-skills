"""SSE chat streaming for AgentCore Harness InvokeHarness."""
    ...
    if event_type == "info":
        ...
        if tool_match:
            ...
            return {
                ...
                "tool": tool_name,
                "input": tool_input,
                "toolUseId": _normalize_tool_use_id(
        ...
        # Also match "Tool Result (name): …" prefix style used by harness client
        if str(data).startswith("Tool Result"):
            return {
                "type": "tool_result",
                "toolUseId": _normalize_tool_use_id(event.get("toolUseId", "")),
                "data": str(data),
    ...
    def _late_persist() -> None:
        ...
        try:
            while True:
                ...
                if remaining <= 0:
                    logger.warning("Late persist timed out waiting for harness worker")
                    return
                try:
        ...
        try:
            ...
            while True:
                ...
                if remaining <= 0:
                    ...
                    logger.warning(
                        "Harness SSE stream timed out after %ss (%s)",
                        elapsed,
            ...
            if "error" in result_holder:
                ...
                yield _sse_event(
                    {"type": "done", "content": error_text, "images": []}
                )
                return
            ...
            if not sse_closed_early and not already_final:
                logger.warning(
                    "SSE client disconnected before harness finished; "
