...
_HARNESS_SYSTEM_PROMPT_BASE = (
    ...
    "- ``share_artifact``는 AgentCore Gateway에 연결된 **MCP 도구**입니다. "
    ...
    "사용자에게 최종 답변하기 **전에** 반드시 MCP 도구 ``share_artifact``를 "
    ...
    "## Agent Workflow\n"
    "1. 사용자 입력을 받는다\n"
    "2. 문서 작성 등은 필요 시 **skill**(docx/pptx 등)을 ``skills``로 로드하고, "
    ...
    "4. 결과 파일이 있으면 사용자 답변 전에 반드시 MCP 도구 ``share_artifact``를 "
    "호출하고, 반환된 공유 URL을 답변에 포함한다 "
    "(로컬 경로만 안내 금지; filepath는 'artifacts/파일명' 또는 ARTIFACTS_DIR 절대경로; "
    "actor_id 필수; ``skills``로 artifact-share를 찾지 말 것)\n"
...
def build_harness_system_prompt(actor_id: str | None = None) -> list[dict]:
    ...
    if aid:
        ...
        text += (
            ...
            "\n"
            "산출물 생성 후 반드시 MCP 도구 ``share_artifact``"
            f"(filepath='{artifacts_dir}/파일명' 또는 'artifacts/파일명', "
            f"actor_id=\"{aid}\")를 직접 호출하고 CloudFront URL을 사용자에게 전달하세요. "
            "``skills`` 도구로 artifact-share를 로드하지 마세요. "
            "로컬 경로만 알려주지 마세요.\n"
            "MCP 도구 ``retrieve``와 ``share_artifact``를 "
