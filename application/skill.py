...
def _is_user_skill(user_id: str | None, name: str) -> bool:
    ...
    Web UI has no S3 Files mount; always resolve user skills via S3
    ``agentcore-sessions/{user}/skills/``.
    """
...
def _list_user_skills_from_s3(user_id: str | None) -> list[dict]:
    ...
    Web UI has no session mount; user skills always come from this S3 prefix.
...
def _list_user_skills(user_id: str | None) -> list[dict]:
    """List skill-creator skills from S3 agentcore-sessions (no app mount)."""
